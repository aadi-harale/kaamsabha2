...
export interface DecisionReceipt { id:string; jobId:string; policyVersion:string; workerId:string; cooperativeId:string; hardChecks:Record<string,boolean>; protectedPayout:number; estimatedCost:number; estimatedNet:number; reason:string; federationReason:string; createdAt:string; candidateWorkerIds?:string[]; }
...
export interface Challenge { id:string; jobId:string; workerId:string; reason:string; status:"open"|"replayed"|"upheld"|"dismissed"; replaySummary?:string; createdAt:string; replayedAt?:string; }
...
export interface AppState { schema:1; revision:number; locale:Locale; session:{userId:string;role:Role}|null; cooperatives:Cooperative[]; workers:Worker[]; jobs:Job[]; receipts:DecisionReceipt[]; opportunities:OpportunityRecord[]; settlements:Settlement[]; cancellations:CancellationRecord[]; safeDeclines:SafeDeclineRecord[]; feedback:Feedback[]; challenges:Challenge[]; issues:IssueRecord[]; suggestions:PolicySuggestion[]; proposals:PolicyProposal[]; policyReviews:{proposalId:string;memberId:string;reviewedAt:string}[]; votes:VoteRecord[]; federation:FederationState; policy:{version:string;minimumPayout:number;maxAddedWaitMinutes:number;activeFrom:string}; }
...
function seededFederation():FederationState{
 ...
 return{opportunities:[{id:"FED-00001",homeCooperativeId:"coop-kharadi",service:"electrician",slaMinutes:35,workerPayout:760,status:"accepted",selectedCooperativeId:"coop-yerawada",workerId:"W01",candidates,federationReceiptId:"FED-RCPT-00001",workerReceiptId:"FED-WORKER-00001",createdAt:seedTime}],snapshots:[{id:"FED-SNAP-00001",opportunityId:"FED-00001",homeCooperativeId:"coop-kharadi",service:"electrician",slaMinutes:35,workerPayout:760,capacities,selectedCooperativeId:"coop-yerawada",workerId:"W01",createdAt:seedTime}],replays:[],settlements:[{id:"FED-SET-00001",opportunityId:"FED-00001",customerTotal:900,workerAmount:760,welfareAmount:40,fulfillingCooperativeAmount:100,reconciles:true,createdAt:seedTime}]};
}
...
{id:"coop-viman",name:"Viman Nagar Labour Cooperative",locality:"Viman Nagar",lat:18.5679,lng:73.9143,active:true,serviceRadiusKm:9,policyVersion:POLICY.version}],workers:[
...
{id:"W09",name:"Manoj Patil",cooperativeId:"coop-hadapsar",skills:["plumbing","cleaning"],certifications:["Plumbing & Leak Safety L2"],verified:true,active:true,available:true,radiusKm:10,workloadTodayMinutes:260,rating:4.6,maxDailyMinutes:480,minRestMinutes:30}],jobs:[],receipts:[],opportunities:[],settlements:[],cancellations:[],safeDeclines:[],feedback:[],challenges:[],issues:[],suggestions:[],proposals:[{id:proposalId,title:"Raise the protected payout floor to ₹860",description:"Increase the minimum protected payout from ₹760 to ₹860 and reduce the maximum fair-work added wait from 12 to 10 minutes.",proposedMinimumPayout:860,proposedMaxAddedWaitMinutes:10,status:"voting",createdAt:seedTime,simulation:{currentFloor:760,proposedFloor:860,currentMaxWait:12,proposedMaxWait:10,workerProtectionDelta:100,affectedJobs:0,note:"Counterfactual only. Past receipts and settlements remain frozen."}}],policyReviews:seededVoters.map(memberId=>({proposalId,memberId,reviewedAt:seedTime})),votes:seededVoters.map(memberId=>({proposalId,memberId,choice:"yes" as const})),federation:seededFederation()};}
