...
function FairWork({ state, job, receipt, worker, run }: { state: AppState; job?: AppState["jobs"][number]; receipt?: AppState["receipts"][number]; worker?: AppState["workers"][number]; run: Run }) {
  ...
  const opportunities=state.opportunities.filter(o=>o.workerId===worker.id);
  ...
  return <><section className="fairHero"><div><p className="eyebrow">FAIR WORK</p><h2>Understand the decision in plain language.</h2><p>{receipt?.federationReason ?? "The home cooperative is checked first; federation is used only when local safe capacity cannot serve the request."} {receipt?.reason ?? "The selected cooperative then applies its own constitution."}</p></div><div className="fairSeal"><span>Decision Receipt</span><strong>{receipt?.id ?? "No receipt yet"}</strong><small>Frozen at dispatch</small></div></section><section className="receiptGrid large"><div><small>Policy</small><strong>{receipt?.policyVersion ?? state.policy.version}</strong><span>Frozen for this decision</span></div><div><small>Protected payout</small><strong>₹{receipt?.protectedPayout ?? state.policy.minimumPayout}</strong><span>No reverse bidding</span></div><div><small>Workload</small><strong>{worker.workloadTodayMinutes} / {worker.maxDailyMinutes ?? 480}</strong><span>Hard safety check</span></div><div><small>Opportunity access</small><strong>{opportunities.length} valid offer{opportunities.length===1?"":"s"}</strong><span>{accepted} accepted · {declined} safe decline{declined===1?"":"s"} · 0 penalty</span></div></section>{opportunities.length>0&&<section className="panel opportunityLedger"><div className="sectionTitle roomy"><div><p className="eyebrow">OPPORTUNITY ACCESS NORMALIZATION</p><h2>Only real eligible offers count.</h2><p className="muted">Workload-blocked or ineligible jobs never become a refusal. A safe decline closes the offer with opportunity penalty 0.</p></div><span className="statusChip">Stored ledger</span></div>{opportunities.slice(0,5).map(o=><div className="opportunityRow" key={o.id}><div><strong>{o.jobId}</strong><span>{humanStatus(o.status)} · hard eligibility passed</span></div><div><small>Opportunity penalty</small><strong>{o.opportunityPenalty}</strong></div></div>)}</section>}<section className="panel replayPanel"><div><div><p className="eyebrow">REPLAY COURT</p><h2>Think this allocation was wrong?</h2><p className="muted">A challenge replays the frozen receipt. It is different from a service issue or a future policy suggestion.</p></div>{job && receipt && <button className="secondary" onClick={() => run(() => openChallenge(state, job.id, "Please replay this allocation using the frozen receipt"), "Replay Court challenge opened.")}>Challenge this decision</button>}</div>{state.challenges.filter((c) => c.workerId === worker.id).map((c) => <div className="challengeRow" key={c.id}><div><strong>{c.id}</strong><span>{c.reason}</span></div><span className="statusChip">{humanStatus(c.status)}</span><p>{c.replaySummary}</p></div>)}</section></>;
}
...
function WorkerGovernance({ state, worker, run }: { state: AppState; worker?: AppState["workers"][number]; run: Run }) {
  ...
  const sim = proposal?.simulation ?? {currentFloor:state.policy.minimumPayout,proposedFloor:860,currentMaxWait:state.policy.maxAddedWaitMinutes,proposedMaxWait:10,workerProtectionDelta:100,affectedJobs:0,note:simulatePolicy(state,860).note};
  const open=proposal?.status==="voting";
  return <><section className="governanceHero"><div><p className="eyebrow">MEMBER GOVERNANCE</p><h2>See what a rule means for you before voting.</h2><p>Suggestions become proposals only after protection review and simulation. Every worker-member gets one vote; dissent stays attached to the record.</p></div><span className="statusChip">Active · {state.policy.version}</span></section><section className="policyImpact"><div><small>CURRENT</small><strong>₹{sim.currentFloor}</strong><span>minimum protected payout</span></div><span>→</span><div><small>PROPOSED</small><strong>₹{sim.proposedFloor}</strong><span>minimum protected payout</span></div><div className="impactPersonal"><small>YOUR PERSONAL IMPACT</small><strong>+₹{sim.workerProtectionDelta} minimum protection per qualifying job</strong><span>Fair-wait allowance {sim.currentMaxWait} → {sim.proposedMaxWait} min. Past receipts stay frozen.</span></div></section><section className="panel"><div className="proposalCard"><div><small>MEMBER BALLOT · {proposal?.status?.toUpperCase()??"PROPOSAL"}</small><h3>{proposal?.title??"Raise the protected payout floor to ₹860"}</h3><p>{proposal?.description??"Policy Twin runs first. Existing jobs never change retroactively."}</p></div>{proposal?.status==="active"?<span className="voteDone">Activated as {state.policy.version}</span>:vote ? <span className="voteDone">Your vote: {vote.choice.toUpperCase()}</span> : reviewed && open ? <div className="memberVoteBox"><div className="actions"><button onClick={() => run(() => castVote(state, proposalId, worker.id, "yes"), "Your yes vote is recorded. One member, one vote.")}>Vote yes</button></div><label className="field"><span>If you oppose, tell members why</span><input value={dissent} onChange={e=>setDissent(e.target.value)} placeholder="Short reason for your no vote" /></label><button className="secondary" disabled={!dissent.trim()} onClick={() => run(() => castVote(state, proposalId, worker.id, "no", dissent), "Your no vote and dissent reason are preserved on the ballot.")}>Vote no with reason</button></div> : open ? <button onClick={() => run(() => reviewPolicyImpact(state, proposalId, worker.id), "Personal impact reviewed. Voting is now available.")}>I reviewed my impact</button>:<span className="voteDone">Ballot closed</span>}</div><div className="governanceSteps"><span className="done">1 Suggest</span><span className="done">2 Validate</span><span className="done">3 Simulate</span><span className={reviewed ? "done" : ""}>4 My impact</span><span className={vote ? "done" : ""}>5 Vote</span><span className={proposal?.status==="active"?"done":""}>6 Quorum & activate</span></div><div className="ballotStats"><div><span>Yes</span><strong>{state.votes.filter((v) => v.proposalId === proposalId && v.choice === "yes").length}</strong></div><div><span>No</span><strong>{state.votes.filter((v) => v.proposalId === proposalId && v.choice === "no").length}</strong></div><div><span>Rule status</span><strong>{proposal?.status==="active"?state.policy.version:"Awaiting member outcome"}</strong></div></div></section></>;
}
...
function Admin({ state, tab, run }: { state: AppState; tab: string; run: Run }) {
  ...
  if (tab === "issues") return <section className="panel"><div className="sectionTitle roomy"><div><h2>Issues & Replay Court</h2><p className="muted">Service concerns and allocation challenges stay separate. Neither automatically deactivates a worker.</p></div><span>{state.issues.length + state.challenges.length}</span></div>{state.issues.map((i) => <div className="caseRow" key={i.id}><div><small>Service / member issue · {i.id}</small><strong>{i.category}</strong><span>{i.jobId ?? "General"} · {i.status}</span>{i.notes.map((n, k) => <p key={k}>{n}</p>)}</div><div className="actions"><button className="secondary" onClick={() => run(() => addIssueNote(state, i.id, "Cooperative review acknowledged."))}>Acknowledge</button>{i.status !== "closed" && <button onClick={() => run(() => reviewIssue(state, i.id, "closed"))}>Close case</button>}</div></div>)}{state.challenges.map((c) => <div className="caseRow" key={c.id}><div><small>Replay Court · {c.id}</small><strong>{c.reason}</strong><span>{c.jobId} · {humanStatus(c.status)}</span><p>{c.replaySummary}</p></div><div className="actions"><span className="statusChip">Frozen receipt</span>{c.status==="open"&&<button className="secondary" onClick={()=>run(()=>replayChallenge(state,c.id),"Frozen allocation receipt replayed. Current worker state was not substituted.")}>Replay frozen decision</button>}</div></div>)}</section>;

  if (tab === "suggestions") return <section className="panel"><div className="sectionTitle roomy"><div><h2>Worker suggestions</h2><p className="muted">A suggestion can enter policy development, but it cannot bypass protection validation, Policy Twin or member voting.</p></div><span>{state.suggestions.length}</span></div>{state.suggestions.length ? state.suggestions.map((s) => <div className="adminSuggestion" key={s.id}><div><small>{s.id} · {s.workerId} · {pretty(s.category)}</small><strong>{s.title}</strong><p>{s.details}</p></div><div><span className={`suggestionStatus ${s.status}`}>{humanStatus(s.status)}</span><div className="actions"><button className="secondary" onClick={() => run(() => reviewPolicySuggestion(state, s.id, "under-review"))}>Review</button><button onClick={() => run(() => reviewPolicySuggestion(state, s.id, "accepted"), "Suggestion accepted for policy development; active constitution unchanged.")}>Accept idea</button><button className="dangerGhost" onClick={() => run(() => reviewPolicySuggestion(state, s.id, "declined"))}>Decline</button></div></div></div>) : <Empty text="No worker policy suggestions yet." />}</section>;
...
function featureDesc(name: string, state: AppState) {
  const descriptions: Record<string, string> = {
    ...
    "Decision Receipt + Replay Court": "Frozen allocation facts + executable challenge replay",
    "Opportunity Access Normalization": `${state.opportunities.length} eligibility-backed offer record(s)`,
    "Protected Payout / No Reverse Bidding": "Pay visible before commitment; paid ranking blocked",
    "Scope Lock + Change Order": "Customer consent before extra scope",
    "Cancellation Protection": "Worker compensation after commitment",
    "Rating & Deactivation Firewall": "Low rating creates human review, not auto-punishment",
    "Workload Safety Guard": "Member workability blocks unsafe offers first",
    "Federation Opportunity Exchange": `${state.federation.opportunities.length} protected two-stage routing proof(s)`,
    "One Member / One Vote": "Personal impact review + quorum precede activation",
