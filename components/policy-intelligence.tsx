...
export function PolicyIntelligence({state,run}:{state:AppState;run:Run}){
 ...
 return <section className="panel policyIntel">
  <div className="policyIntelHead"><div><p className="eyebrow">COLLECTIVE POLICY SIGNAL MONITOR</p><h2>Find repeated worker concerns before they become invisible patterns.</h2><p className="muted">AI reads anonymized issue and suggestion text, clusters themes and drafts discussion language. It cannot rank workers, punish anyone, dispatch, vote, set pay or activate policy.</p></div><button disabled={busy||sourceIssues.length+sourceSuggestions.length===0} onClick={()=>void analyze()}>{busy?"Analyzing member voice…":"Analyze all member voice"}</button></div>
